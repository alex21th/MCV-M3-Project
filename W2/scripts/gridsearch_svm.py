...
from scripts.utils import create_feat_des
from scripts.utils import spatial_pyramid_des
from scripts.utils import spatial_pyramid_histograms


# WITHOUT Dimensionality Reduction
def set_params(feat_des='sift',dense=False,level=0,k=128,nfeatures=300,hessianThreshold=600,
               step_div=20,num_sizes=1,pca_perc=0.8, lda_ncomponents=7, norm='None', subregion_shape='square'):
    params = {
        ...
        'num_sizes': num_sizes,
        'pca_perc': pca_perc,
        'lda_ncomponents': lda_ncomponents,
        'norm': norm,
...
def get_descriptors_D(feat_des_options):
    train_images_filenames = cPickle.load(open('../MIT_split/train_images_filenames.dat','rb'))
    test_images_filenames = cPickle.load(open('../MIT_split/test_images_filenames.dat','rb'))
    train_labels = cPickle.load(open('../MIT_split/train_labels.dat','rb'))
    test_labels = cPickle.load(open('../MIT_split/test_labels.dat','rb'))
...
def run(train_pyramid_descriptors, D, test_pyramid_descriptors, feat_des_options):
    ...
    svm = SVC()
    scores = cross_validate(svm, visual_words_pyramid, train_labels, scoring = ['accuracy'], cv=8,return_estimator=True)
